...
function updateGpaResults() {
  ...
  // 2. Overall CGPA (credit-weighted across all entered semesters)
  ...
  for (let i = 1; i <= TOTAL_SEMS; i++) {
    ...
    if (tc > 0) {
      allCredits += tc;
      allPoints += tp;
      allSemGpas.push({ sem: i, gpa: tp / tc, credits: tc });
    }
  }
  ...
  if (summaryEl) {
    if (allSemGpas.length === 0) {
      ...
      allSemGpas.forEach(({ sem, gpa }) => {
        html += `<span>Sem ${sem}: <strong style="color:#2563eb;">${gpa.toFixed(2)}</strong></span>`;
      });
      html += `</div>`;
      if (cgpa !== null) {
        html += `<div style="font-weight:800;color:#059669;">Overall CGPA: ${cgpa.toFixed(2)} / 10.0 (${allCredits} Credits)</div>`;
      }
...
async function syncGpaToStudentProfile() {
  ...
  for (let i = 1; i <= TOTAL_SEMS; i++) {
    ...
    if (g !== null) {
      const subs = gpaData[i] || [];
      const tc = subs.reduce((sum, s) => sum + (parseFloat(s.credits) || 0), 0);
      allCredits += tc;
      allPoints += tc * g;
    }
  }

  const calculatedCgpa = allCredits > 0 ? (allPoints / allCredits).toFixed(2) : '0.00';

  if (!confirm(`Sync calculated CGPA (${calculatedCgpa}) to your placement profile?`)) return;
